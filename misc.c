...
int chldrn(int proc_nr, int level)
{
    int children = 0;
    if (level == 0)
        return 0;
    level--;
    for (int i = 128; i <= NR_PROCS; i++)
        if ((mproc[i].mp_flags & IN_USE) && (mproc[i].mp_parent == proc_nr) && (i != proc_nr)) {
	    children++;
            children = children + chldrn(i, level);
	}
...
int do_maxDesc(void)
{
    int level = m_in.m1_i1;
    int maxChildren = 0;
    for (int proc_nr = 128; proc_nr <= NR_PROCS; proc_nr++)
    {
        if (mproc[proc_nr].mp_flags & IN_USE)
        {
            int children = chldrn(proc_nr, level);
            if (children > maxChildren)
            {
                maxChildren = children;
            }
